...
def match_visual_boxes_to_anchors(anchors, visual_boxes):
    ...
    for anchor in anchors:
        ...
        # Cari kotak yang berada di atas anchor
        candidates = [vb for vb in visual_boxes if vb["y"] + vb["height"] < anchor_y]

        if candidates:
            best_match = min(candidates, key=lambda vb: anchor_y - (vb["y"] + vb["height"]))
            matched_zones.append({
                "anchor": anchor["text"],
                "zone_box": best_match
            })
        else:
            print(f"⚠️ No visual box found ABOVE anchor {anchor['text']}")
...
def main(image_path, ocr_json_path, output_json_path):
    ...
    print(f"Anchor-downward matched zones saved to {output_json_path}")

if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python detect_placard_zones_anchor_top.py <image_path> <ocr_json_path> <output_json_path>")
        sys.exit(1)
