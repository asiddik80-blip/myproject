...
// === SAVE FIX FUNCTION ===
function saveDocumentNoDialog(doc, outputPath) {
    var saveFile = new File(outputPath);
    var opts = new IllustratorSaveOptions();

    // Pakai format Illustrator terbaru
    opts.compatibility = Compatibility.ILLUSTRATORLATEST;

    // Matikan PDF compatibility → hindari legacy dialog
    ...
    // Close tanpa prompt lagi
    doc.close(SaveOptions.DONOTSAVECHANGES);
}

// === MAIN PLACE FUNCTION ===
function placeContentOnArtboards(targetDoc, inputPages, artWidth, artHeight) {
    writeLog("=== PLACING CONTENT ON ARTBOARDS ===");

    for (var i = 0; i < inputPages.length; i++) {
        var pagePath = inputPages[i];
        writeLog("Processing page " + (i + 1) + "/" + inputPages.length);

        // Tambah artboard untuk page ini
        if (i > 0) {
            var rect = [0, artHeight, artWidth, 0];
            targetDoc.artboards.add(rect);
        }

        targetDoc.artboards.setActiveArtboardIndex(i);

        // === Page 1 sebagai IMAGE ===
        if (i === 0) {
            var placed = targetDoc.placedItems.add();
            placed.file = new File(pagePath);

            // Fit ke artboard (seperti Script B)
            var ab = targetDoc.artboards[i].artboardRect;
            var abW = ab[2] - ab[0];
            var abH = ab[1] - ab[3];
            var scaleX = abW / placed.width;
            var scaleY = abH / placed.height;
            var scale = Math.min(scaleX, scaleY);
            placed.width = placed.width * scale;
            placed.height = placed.height * scale;
            placed.position = [ab[0], ab[1]];

        // === Page 2..n sebagai VEKTOR ===
        } else {
            var pdfFile = new File(pagePath);
            var pdfOpts = new PDFFileOptions();
            var grp = targetDoc.groupItems.createFromFile(pdfFile, pdfOpts);

            // Centering grup ke artboard
            var ab = targetDoc.artboards[i].artboardRect;
            var abW = ab[2] - ab[0];
            var abH = ab[1] - ab[3];
            ...
            var gW = b[2] - b[0];
            var gH = b[1] - b[3];
            var offsetX = ab[0] + (abW - gW) / 2 - b[0];
            var offsetY = ab[1] - (abH - gH) / 2 - b[1];
...
// === MAIN RUN ===
function main() {
    var files = {{FILES_ARRAY}}; // Laravel inject

    for (var f = 0; f < files.length; f++) {
        var inputPages = files[f].input;
        var outputFilePath = files[f].output;
        var artWidth = files[f].width;
        var artHeight = files[f].height;

        writeLog("=== START FILE === " + outputFilePath);

        var doc = app.documents.add(DocumentColorSpace.CMYK, artWidth, artHeight);
        placeContentOnArtboards(doc, inputPages, artWidth, artHeight);

        saveDocumentNoDialog(doc, outputFilePath);
        writeLog("Saved to " + outputFilePath);
    }
